...
def dev_skill_language():
    ...
    return render_template('skills.html', developer = Developer.get_skill_by_developer(data), cant_Skill = verificarCantidadSkill[0]['count'], dev_shortBio = Developer.get_one(data))
...
def topfiveskill_lang(skill_id,name_skill):
    ...
    # Verifica la cantidad de 'Skill' que tiene el developer actual
    verificarCantidadSkill = Count.getCountSkill_lang_by_developer(data_skill_of_developer)
...
def topfiveskill_fram():
    return render_template('frameworks.html')


@app.route('/delete_Skill/<int:skill_id>/<string:name_skill>')
def delete_Skill(skill_id, name_skill):
